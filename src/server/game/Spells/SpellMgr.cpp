...
void SpellMgr::LoadSpellProcs()
{
    ...
    for (SpellInfo* spellInfo : mSpellInfoMap)
    {
        ...
        auto checkAuraType = [&procEntry, spellInfo](AuraType aurType) -> bool
        {
            if (spellInfo->HasAura(aurType))
            {
                for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                    if (spellInfo->Effects[i].IsAura(aurType))
                        procEntry.SchoolMask |= spellInfo->Effects[i].MiscValue;
        ...
        if (!checkAuraType(SPELL_AURA_REFLECT_SPELLS_SCHOOL) &&
            !checkAuraType(SPELL_AURA_MOD_POWER_COST_SCHOOL_PCT) &&
            !checkAuraType(SPELL_AURA_MOD_POWER_COST_SCHOOL))
            procEntry.SchoolMask = spellInfo->SchoolMask;

        procEntry.SpellFamilyName = spellInfo->SpellFamilyName;
        procEntry.SpellFamilyMask = spellInfo->SpellFamilyFlags;
        procEntry.ProcFlags       = spellInfo->ProcFlags;
        procEntry.SpellTypeMask   = PROC_SPELL_TYPE_MASK_ALL;
        procEntry.SpellPhaseMask  = PROC_SPELL_PHASE_FINISH;
