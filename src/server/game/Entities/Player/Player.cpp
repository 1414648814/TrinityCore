...
void Player::CastItemCombatSpell(Unit* target, CalcDamageInfo const& damageInfo) /* WeaponAttackType attType, uint32 procVictim, uint32 procEx)*/
{
    ...
    for (uint8 i = EQUIPMENT_SLOT_START; i < EQUIPMENT_SLOT_END; ++i)
    {
        ...
        {
            if (!item->IsBroken() && CanUseAttackType(dmgInfo.GetAttackType()))
            {
                if (ItemTemplate const* proto = item->GetTemplate())
                {
                    ...
                    if (proto->Class == ITEM_CLASS_WEAPON)
                    {
                        ...
                        if (dmgInfo.GetAttackType() == BASE_ATTACK)
                            if (!IsUseEquipedWeapon(true) && !IsInFeralForm())
                                continue;
                    }

                    CastItemCombatSpell(target, dmgInfo, item, proto);
                }
            }
        }
    }
...
void Player::CastItemCombatSpell(Unit* target, DamageInfo const& damageInfo, Item* item, ItemTemplate const* proto) /* WeaponAttackType attType, uint32 procVictim, uint32 procEx */
{
